...
from app.utils.email_sender import get_default_email_sender, get_mailtrap_email_sender, get_bizflycloud_email_sender
...
@activity.defn
async def send_email_smtp_activity(email: str, otp: str, provider: str = "default") -> bool:
    """
    Send OTP email to user using smtplib.SMTP.

    Args:
        email: Recipient email address
        otp: One-time password to send
        provider: Email provider to use ('default', 'mailtrap', or 'bizflycloud')

    Returns:
        bool: True if email sent successfully, False otherwise
    """
    try:
        # Select email sender based on provider
        if provider == "mailtrap":
            email_sender = get_mailtrap_email_sender()
        elif provider == "bizflycloud":
            email_sender = get_bizflycloud_email_sender()
        else:
            email_sender = get_default_email_sender()
        ...
        if result:
            activity.logger.info(f"OTP email sent successfully to {email} using SMTP ({provider})")
        else:
            activity.logger.error(f"Failed to send OTP email using SMTP ({provider})")
            
        return result
    except Exception as e:
        activity.logger.error(f"Failed to send OTP email using SMTP: {str(e)}")
        return False
